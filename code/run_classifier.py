...
logger.setLevel(logging.INFO)  # TODO changed here
...
def train(args, train_dataset, model, tokenizer):
    ...
    args.save_steps = len(
        train_dataloader) if args.save_steps <= 0 else args.save_steps
    args.warmup_steps = len(
        train_dataloader) if args.warmup_steps <= 0 else args.warmup_steps
    args.logging_steps = len(train_dataloader)

    if args.max_steps > 0:

        t_total = args.max_steps//10
    ...
    best_results = {"acc": 0.0, "precision": 0.0,
                    "recall": 0.0, "f1": 0.0, "acc_and_f1": 0.0}
    model.zero_grad()
...
def main():
    ...
    if args.checkpoint_path:
        model.load_state_dict(torch.load(os.path.join(
            args.checkpoint_path, 'pytorch_model.bin')))
    ...
    if args.do_train:
        ...
        train_dataset = TextDataset(
            tokenizer, args, train_data_path, type='train')
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        logger.info("Eval Model From: {}".format(
            os.path.join(output_dir, 'pytorch_model.bin')))
        logger.info("***** Eval results *****")
